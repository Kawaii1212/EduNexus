...
public class QuestionController : Controller
{
    ...
    public IActionResult Details(long id)
    {
        ...
        if (q == null)
        {
            TempData["ErrorMessage"] = "Không tìm th?y câu h?i.";
    ...
    public IActionResult Create(QuestionFormViewModel form)
    {
        ...
        // Gi? l?p userId = 1 khi chua có Auth hoàn ch?nh
        long creatorId = 1;
        ...
        try
        {
            ...
            TempData["SuccessMessage"] = "? Thêm câu h?i th? công thành công.";
    ...
    public IActionResult Edit(long id, QuestionFormViewModel form)
    {
        ...
        if (q == null)
        {
            TempData["ErrorMessage"] = "Không tìm th?y câu h?i d? c?p nh?t.";
            return RedirectToAction(nameof(Index));
        }
        ...
        try
        {
            ...
            TempData["SuccessMessage"] = "? C?p nh?t câu h?i thành công.";
            return RedirectToAction(nameof(Index), new { moduleId = form.ModuleId });
        }
        catch (Exception ex)
        {
    ...
    public IActionResult Delete(long id)
    {
        ...
        if (q == null)
        {
            TempData["ErrorMessage"] = "Không tìm th?y câu h?i d? xoá.";
            return RedirectToAction(nameof(Index));
        }
        ...
        try
        {
            ...
            TempData["SuccessMessage"] = "?? Ðã xoá câu h?i thành công.";
        }
        catch (Exception ex)
        {
            TempData["ErrorMessage"] = $"? Không th? xoá câu h?i này: {ex.Message}";
        }
    ...
    public async Task<IActionResult> Import(QuestionImportViewModel model)
    {
        if (model.ModuleId <= 0)
        {
            ModelState.AddModelError(nameof(model.ModuleId), "Vui lòng ch?n Module nh?n câu h?i.");
        }
        ...
        {
            ModelState.AddModelError("", "Vui lòng upload file .csv ho?c dán n?i dung CSV.");
            return View(model);
        }
        ...
        foreach (var rawLine in lines)
        {
            ...
            // B? qua dòng tiêu d? n?u dòng d?u trùng d?nh d?ng c?t m?u
            ...
            if (columns.Count < 6)
            {
                errors.Add($"Dòng {rowNum}: Thi?u d? li?u (C?n ít nh?t 6 c?t: N?i dung, A, B, C, D, Ðáp án dúng).");
                continue;
            }
            ...
            {
                errors.Add($"Dòng {rowNum}: N?i dung câu h?i không du?c d? tr?ng.");
                continue;
            }
            ...
            {
                errors.Add($"Dòng {rowNum}: Các l?a ch?n A, B, C, D không du?c d? tr?ng.");
                continue;
            }
            if (correctOpt != "A" && correctOpt != "B" && correctOpt != "C" && correctOpt != "D")
            {
                errors.Add($"Dòng {rowNum}: Ðáp án dúng '{correctOpt}' không h?p l? (ph?i là A, B, C ho?c D).");
                continue;
            }
        ...
        {
            errors.Add("Không tìm th?y d? li?u h?p l? d? import.");
        }
        ...
        try
        {
            ...
            TempData["SuccessMessage"] = $"? Import thành công {questionsToImport.Count} câu h?i vào ngân hàng câu h?i.";
    ...
    // Helper tách dòng CSV h? tr? d?u ngo?c kép
    private static List<string> ParseCsvLine(string line)
    {
        ...
        for (int i = 0; i < line.Length; i++)
        {
            ...
            if (c == '"')
            {
                // X? lý d?u ngo?c kép l?ng nhau "" d?i di?n cho "
